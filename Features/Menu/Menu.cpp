	...
	if (!bDone)
	{
		m_ItemLists =
		{
				...
				"Aimbot",
			{
				new CItemGroup{
					"Main", {
							new CItemBool("Active", &Vars::Aimbot::Enabled)
}
}
			}
			}
...
void CMenu::Run()
{
	...
	if (m_Open)
	{
		...
		for (size_t nItemList = 0; nItemList < m_ItemLists.size(); nItemList++)
		{
			...
			//name
			G::Draw.String(EFonts::MENU_TAHOMA, (ListWidth * nItemList) + (ListWidth / 2), (ListBarH / 2), Text, TXT_DEFAULT,
			...
			for (size_t nItemGroup = 0; nItemGroup < ItemList->m_ItemGroups.size(); nItemGroup++)
			{
				...
				//name
				G::Draw.String(EFonts::MENU_TAHOMA, nGroupX + (ListWidth / 2) + 1, nGroupY + GroupH / 2, Text, TXT_DEFAULT,
				...
				if (ItemGroup->m_Open)
				{
					for (size_t nItems = 0; nItems < ItemGroup->m_Items.size(); nItems++)
					{
						...
						case EItemType::INT:
						{
							...
							//name
							G::Draw.String(EFonts::MENU_TAHOMA, nDrawX, nDrawY, DrawCol, TXT_DEFAULT, "%s", Int->m_Name.c_str());
							...
							if (!Int->m_Aliases.empty())
							{
								...
								//value
								G::Draw.String(EFonts::MENU_TAHOMA, nDrawValX, nDrawY, DrawCol, TXT_DEFAULT, "%s",
							...
							else
							{
								...
								//value
								G::Draw.String(EFonts::MENU_TAHOMA, nDrawValX, nDrawY, DrawCol, TXT_DEFAULT, "%d",
						...
						case EItemType::FLOAT:
						{
							...
							//name
							G::Draw.String(EFonts::MENU_TAHOMA, nDrawX, nDrawY, DrawCol, TXT_DEFAULT, "%s",
								Float->m_Name.c_str());
							...
							//value
							G::Draw.String(EFonts::MENU_TAHOMA, nDrawValX, nDrawY, DrawCol,TXT_DEFAULT, "%.2f",
						...
						case EItemType::BOOL:
						{
							...
							//name
							G::Draw.String(EFonts::MENU_TAHOMA, nDrawX, nDrawY, DrawCol, TXT_DEFAULT, "%s", Bool->m_Name.c_str());
