...
void CAimbot::Projectile(C_TFPlayer* local_player, C_TFPlayer* entity, C_BaseCombatWeapon* local_weapon, Vector& vec_hitbox)
{
...
#pragma endregion
	...
	int iBestHitbox = GetBestHitbox(pLocal, pEntity->As<C_TFPlayer*>());
	...
	Projectile(pLocal, pEntity->As<C_TFPlayer*>(), pWeapon, vEntity);
...
int CAimbot::GetBestTarget(C_TFPlayer* pLocal, CUserCmd* pCommand)
{
	...
	for (int i = 1; i <= I::EngineClient->GetMaxClients(); i++)
	{
		...
		auto pEntity = iEntity->As<C_TFPlayer*>();
...
int CAimbot::GetBestHitbox(C_TFPlayer* pLocal, C_TFPlayer* pEntity)
{
	int iBestHitbox = -1;
